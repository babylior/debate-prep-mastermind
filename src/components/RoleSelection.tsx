...
const RoleSelection: React.FC<RoleSelectionProps> = ({ onComplete }) => {
  ...
  const getTeamColorClass = (team: TeamPosition) => {
    switch(team) {
      case 'OG': return 'bg-green-500';
      case 'OO': return 'bg-blue-500';
      case 'CG': return 'bg-yellow-500';
      case 'CO': return 'bg-red-500';
  ...
  const getTeamBorderClass = (team: TeamPosition) => {
    switch(team) {
      case 'OG': return 'border-green-500';
      case 'OO': return 'border-blue-500';
      case 'CG': return 'border-yellow-500';
      case 'CO': return 'border-red-500';
  ...
  const getTeamBgClass = (team: TeamPosition) => {
    switch(team) {
      case 'OG': return 'bg-green-50';
      case 'OO': return 'bg-blue-50';
      case 'CG': return 'bg-yellow-50';
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-8">
      <Card className="border-t-4 border-t-primary">
        <CardHeader>
          ...
          <CardDescription>Enter the motion to begin your prep (optional)</CardDescription>
        </CardHeader>
      ...
      <div className="text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {Object.entries(teamPositions).map(([team, label]) => (
            <button
              key={team}
              onClick={() => handleTeamSelect(team as TeamPosition)}
              className={`
      ...
      {selectedTeam && (
        <div className="bg-white p-6 rounded-lg shadow-sm border animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {rolesByTeam[selectedTeam]
              .sort((a, b) => a.order - b.order)
              .map((role) => (
                <Button
                  key={role.id}
                  variant={selectedRole === role.id ? "default" : "outline"}
                  className={`justify-start ${selectedRole === role.id ? role.teamColor : 'hover:bg-gray-100'}`}
